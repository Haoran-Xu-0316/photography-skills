...
对底图生成3张横向互补清晰区域：高斯模糊sigma=5，以x=170、575、980像素为中心、sigma=185的权重恢复各自清晰区域。没有真实改变镜头焦距或焦平面。
...
3个输入均贡献了清晰区域，份额约32.3%、34.9%、32.8%，风险等级low。已对照各帧和融合画面，清晰区域合成有效。来源图是选帧索引，不是深度图；本例不能证明真实焦点呼吸或复杂遮挡处理。

![输入与实际结果](comparison.jpg)

[清晰来源图](output/focus_source_map.png)；[完整输出](output/)；[验证数据与输入哈希](verification.json)。
