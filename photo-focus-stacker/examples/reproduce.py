...
engine = import_module("photo_focus_stacker")
...
def reproduce_example(output_directory: str | Path) -> dict:
    ...
    paths = [INPUT / f"focus-{index}.png" for index in range(1, 4)]
    analysis = engine.analyze_focus_stack(paths)
    if analysis["blockers"]:
        return {"analysis": analysis, "result": None}
    return {"analysis": analysis, "result": engine.stack_focus(paths, output_directory, blend_radius=7)}
